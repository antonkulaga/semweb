...
trait LoadGenAge extends GeneSchema{
  ...
  lazy val genAgePairs: Map[String, IRI] = Map("cDB" ->adb,
  ...
  lazy val annotationPairs: Map[String, IRI] = Map("DB" ->adb,
    "DB Object Name" ->dbObjectName,
    "DB Object ID"-> objId,
    "DB Object Symbol"->symbol,
    "ENTREZID"-> entrezId,
  ...
  def parse(property:IRI,string:String,base:Option[IRI])(implicit shape:Shape):Option[RDFValue] =  {
   val v = shape.arcRules().collect{
      ...
      if(results.exists(r=>r==value || r.label==value)) value else {
        println(s"$string is not in the valueset")
        value
      }*/
      case ValueStem(stem) =>
      ...
      case ValueType(t) => t match {
        ...
        case XSD.IntDatatypeIRI | XSD.IntegerDatatypeIRI =>
          Try(IntLiteral(string.toInt))
            .recover{case result=> println(s"error in parsing Int of $string") ; StringLiteral(string)}.get:RDFValue

        case XSD.DecimalDatatypeIRI | XSD.DoubleDatatypeIRI =>
          Try(DoubleLiteral(string.toDouble))
            .recover{case result=> println(s"error in parsing Double of $string") ; StringLiteral(string)}.get:RDFValue
        ...
        case XSD.Date =>
          DateTimeFormats.parseDate(string).map{
            ...
            println("ERROR = cannot parse following date: "+string)
    ...
    if(v.isEmpty)  println("ERROR =DOES NOT WORK WITH"+property.stringValue+" with value = "+string)
 ...
 def readFromFile(path:String) =  Source.fromFile(path).getLines().reduce(_+"\n"+_)

...
import org.denigma.semweb.rdf.{Trip, Quad, Res}
...
case class OrRule(disjoints: Set[Rule], id:Label) extends RuleContainer {
...
case class GroupRule(rule: Rule, opt: Boolean, a: Set[Action], id:Label) extends Rule
{
  def toQuads(subject:Res)(implicit context:Res = null):Set[Quad] = ???

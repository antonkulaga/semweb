...
import org.denigma.semweb.parsers.sample.BasicElementsParser
...
class TestRdfTermParser(input:ParserInput) extends BasicElementsParser(input)
{
  ...
  def BooleanLiteral = rule {
    (TRUE~push(org.denigma.semweb.rdf.BooleanLiteral(true))) | (FALSE~push(org.denigma.semweb.rdf.BooleanLiteral(false)))
  }
  ...
  def BLANK_NODE_LABEL()  = rule{
    capture("_:" ~ PN_LOCAL)~>(v=>org.denigma.semweb.rdf.BlankNode(v)) ~ WS
  }
      ...
      capture(
        zeroOrMore(
          !(
            ...
            ) ~ ANY)) ~>(v=>org.denigma.semweb.rdf.IRI(v)) ~   GREATER
  }
